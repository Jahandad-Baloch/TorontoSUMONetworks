...
class XMLGenerator:
    ...
    def process_traffic_data(self, grouped_traffic_data: pd.DataFrame, junctions_with_directions_df: pd.DataFrame, intervals: Dict[str, ET.Element], connections: dict, mode: str):
        ...
        for (time_start, time_end), group in grouped_traffic_data:
            ...
            if interval_id in intervals:
                ...
                for _, traffic_data in group.iterrows():
                    node_id = str(traffic_data['centreline_id'])

                    junction_row = junctions_with_directions_df[junctions_with_directions_df['junction_id'] == node_id]
                    ...
                    for edge_id, direction in zip(edges, directions):
                        ...
                        if edge_id in connections:
                            for to_edge, turn_dir in connections[edge_id]:
                                feature_name = f"{direction}_{mode}_{turn_dir}"
                                if feature_name in traffic_data:
                                    count = traffic_data[feature_name]
                                    edge_relation_id = (edge_id, to_edge, count)

                                    if edge_relation_id not in processed_edge_relations:
