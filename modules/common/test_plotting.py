...
class NetworkParser:
    ...
    """
    def __init__(self, network_file: str, logger=None) -> None:
    ...
    def load_network(self) -> None:
        ...
        print("Junctions parsed.")

        # for tls in net_tls:
        #     self._parse_tllogic(tls) # check this method later
        
        print(f"Loaded SUMO network elements from: {self.network_file}")
    ...
    def plot_network2(self):
        """
        Generates a network snapshot using an optimized vectorized approach via sumolib.
        """
        net_file_path = "data/sumo_networks/scarborough-guildwood/scarborough-guildwood_arterial.net.xml"
        # Utilize sumolib to extract lane shapes in a vectorized manner.
        segments = [lane.getShape() for edge in self.net.getEdges() 
                                  for lane in edge.getLanes() 
                                  if len(lane.getShape()) > 1]
        lc = LineCollection(segments, colors='blue', linewidths=0.5)
        fig, ax = plt.subplots(figsize=(10, 10))
        ax.add_collection(lc)
        if self.junctions:
            xs = [node['x'] for node in self.junctions.values()]
            ys = [node['y'] for node in self.junctions.values()]
            ax.scatter(xs, ys, c='red', s=10)
        ax.autoscale_view()
        ax.set_aspect('equal', 'box')
        snap_path = "data/sumo_networks/scarborough-guildwood/network_snaps1.png"       
        plt.savefig(snap_path, dpi=150)
        plt.close()
    
# Debug: Test the network parser
net_file_path = "data/sumo_networks/scarborough-guildwood/scarborough-guildwood_arterial.net.xml"
network_parser = NetworkParser(net_file_path)
network_parser.load_network()
# network_parser.plot_network()
network_parser.plot_network2()
# Debug: "Network plot saved."